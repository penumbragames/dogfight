...
Drawing.create = function(context) {
  return new Drawing(context, scene);
}
...
Drawing.prototype.redrawOtherPlayers = function(otherPlayers) {
  ...
  for (var i = 0; i < otherPlayers.length; i++) {
    var playerGeometry = new THREE.BoxGeometry(otherPlayers[i].size[0] * 2,
                                               otherPlayers[i].size[1] * 2,
                                               otherPlayers[i].size[2] * 2);
...
Drawing.prototype.redrawBullets = function(bullets) {
  ...
  for (var i = 0; i < bullets.length; i++) {
    var bulletGeometry = new THREE.BoxGeometry(bullets[i].size[0] * 2,
                                               bullets[i].size[1] * 2,
                                               bullets[i].size[2] * 2);
...
Drawing.prototype.redrawMissiles = function(missiles) {
  ...
  for (var i = 0; i < missiles.length; i++) {
    var missileGeometry = new THREE.BoxGeometry(missiles[i].size[0] * 2,
                                               missiles[i].size[1] * 2,
                                               missiles[i].size[2] * 2);
