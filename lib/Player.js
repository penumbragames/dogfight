...
function Player(socketId, name, position, orientation, timeCreated) {
  ...
  this.weapon = Constants.PLAYER_WEAPONS.BULLET;
 ...
 * @param {boolean} fire Whether or not the player is firing their weapon.
 * @param {function(*)} fireCallback The callback function
 */
Player.prototype.updateOnInput = function(controls, mouseControl, fire,
                                          fireCallback) {
  ...
  if (fire && this.canShoot()) {
    fireCallback();
    this.lastWeaponTime[this.weaponState] = (new Date()).getTime();
  }
};
...
Player.prototype.update = function() {
  ...
  this.setVX(Math.sin(this.getPitch()) * Math.sin(this.getYaw()) * this.vmag);
  this.setVY(Math.sin(this.getPitch()) * Math.cos(this.getYaw()) * this.vmag);
  this.setVZ(Math.cos(this.getPitch()) * this.vmag);

  var tmpRoll = this.getRoll();
  if (tmpRoll > Math.PI) {
    tmpRoll = this.getRoll() - (2 * Math.PI);
  }
  this.setYaw(this.getYaw() + (tmpRoll * 0.1));
};
...
Player.prototype.canShoot = function() {
  return (new Date()).getTime() > (this.lastWeaponTime[this.weapon] +
      Player.COOLDOWNS[this.weapon]);
};
