 ...
 *   along each axis in radians. [pitch, yaw, roll].
...
Entity.prototype.setPitch = function(pitch) {
  this.orientation[0] = Util.bound(0, 2 * Math.PI, pitch);
};
...
Entity.prototype.setYaw = function(yaw) {
  this.orientation[1] = Util.bound(-Math.PI, Math.PI, yaw);
};
...
Entity.prototype.setRoll = function(roll) {
  this.orientation[2] = Util.bound(0, 2 * Math.PI, roll);
};
